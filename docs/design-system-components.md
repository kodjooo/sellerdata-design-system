...
| **DsCopyField** | `value` (required), `label`, `break` (перенос длинного значения) | поле «значение + копирование» (DsCopyButton): опц. подпись + бокс border/субтл-фон; реал — реферальная ссылка в «Пригласи друга» | `resources/js/Components/Ds/DsCopyField.vue` |
| **DsCallout** | `title`, `text`; слоты `action`/default | субтл-плашка-CTA: заголовок + текст + действие справа, перенос на узких; реал — партнёрская программа в «Пригласи друга» | `resources/js/Components/Ds/DsCallout.vue` |
| **DsPromoCard** | `badge`, `icon` (fm-* декор), `badgeTone` pink (def pink); slot default | градиентная промо-карточка: декор-иконка в углу + круглый бейдж + текст снизу; высоту задаёт раскладка страницы; реал — «+1 месяц бесплатно» | `resources/js/Components/Ds/DsPromoCard.vue` |
| **DsStepFlow** | `steps` [{icon (fm-*), title, text}] | горизонтальный поток иконочных шагов со стрелками-разделителями; на <lg — колонка, стрелки скрыты; реал — «Как это работает» в «Пригласи друга» | `resources/js/Components/Ds/DsStepFlow.vue` |
